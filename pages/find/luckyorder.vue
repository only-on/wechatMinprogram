	...
	<view>
		...
		<view class="goods-section">
			...
			<view class="g-item">
				<image :src="url"></image>
				<view class="right">
					<text class="title clamp">{{ name }} </text>
					<text class="spec">&nbsp;&nbsp;&nbsp;[48小时内发货]&nbsp;&nbsp;[送货到家]</text>
					<view class="price-box">
						<text class="price" style="color: #ff1048;">￥{{ price }}</text>
					</view>
				</view>
			</view>
		</view>
		<view class="yt-list">
			...
			<view class="yt-list-cell b-b">
				...
				<view class="example-body" style="margin-left: 200upx;">
					<uni-number-box :min="1" :max="1" :value="numberValue" />
				</view>
			</view>
			<view class="yt-list-cell b-b">
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price" v-if="price <= 0">0.01</text>
			</view>
...
<script>
	...
	export default {
		...
		async onLoad(option) {
			//商品数据
			that = this;
			that.raffleJournalId = option.id;
			that.loadData();
		},
		methods: {
			change(value) {
					...
					if (-1 != that.orderidnumber) {
						uni.request({
							...
							success: res => {
								...
								console.log('success:' + res.data.nonceStr);
								uni.requestPayment({
									provider: 'wxpay',
						...
						uni.request({
							url: that.$api_url + '/order/create', //仅为示例，并非真实接口地址。
							method: 'POST',
							header: {
								'authorization': uni.getStorageSync('authorization') //自定义请求头信息
							},
							data: {
								customer_id: uni.getStorageSync('openid'),
								type: 4,
							...
							success: res => {
								...
								if (1000 == res.data.code) {
									...
									// });
									that.orderidnumber = res.data.content.id
									uni.request({
										url: this.$api_url + '/weixin/payment.do', //仅为示例，并非真实接口地址。
										method: 'POST',
										header: {
										...
										success: res => {
											...
											// nonceStr: "8a28b8de713a575801713a575a670001"
											// package: "prepay_id=wx02180215454541b895e6f4921141301600"
											// paySign: "CC92782F19A99A22194EA3E636C44AAA"
											// timeStamp: "1585821735"
											// 仅作为示例，非真实参数信息。
											console.log('success:' + JSON.stringify(res.data));
											console.log('success:' + res.data.nonceStr);
											uni.requestPayment({
